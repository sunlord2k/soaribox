...
class watchdog:
    def __init__(self, whattowatch, *args):
        nmea_timeout = 0
        influx_timeout = 0
        gpspoller_timeout = 0
            ...
            for i in range(0, 6):
                if self.alive[i]:
                    ...
                    if self.timeout[i] > 15:
                        ...
                        logging.critical(now.strftime("%Y-%m-%d %H:%M:%S") + "The Watchdog was triggered and killed all Threads")
        ...
        try:
            while True:
                ...
                if watchdog_Thread_kill_register[1] == 1:
                    pass
#                    raise ValueError('The Watchdog killed the GPS thread')
...
#                    raise ValueError('The Watchdog killed the GPS sensor feeding thread')
    ...
    #  to be implemented            nmea_GSA = pynmea2.GSA('GP', 'GSA', ('A', '3', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '1.0', '1.0', '1.0'))
                crlf = "\r\n"
                sock.sendto(bytes(nmea_GGA), (c.UDP_IP, c.UDP_PORT))  # Send
                sock.sendto(bytes(crlf), (c.UDP_IP, c.UDP_PORT))
                if nmea_track != 'nan':
                    sock.sendto(bytes(nmea_RMC), (c.UDP_IP, c.UDP_PORT))
                    sock.sendto(bytes(crlf), (c.UDP_IP, c.UDP_PORT))
        ...
        #        sock.sendto(bytes(crlf), (UDP_IP, UDP_PORT))
                ...
                if watchdog_Thread_kill_register[0] == 1:
                    ...
                    break
#                    raise ValueError('The Watchdog killed the NMEA out thread thread')
